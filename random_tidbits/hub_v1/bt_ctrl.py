...
class Bluetooth_Controller:
	...
	def setup_new_sensor(self, sensor):
		print "Setting up new sensor"
		new_dev = self.find_device_in_objects(sensor.address)
		
		if not(new_dev.Get('org.bluez.Device1', 'Connected')):
			new_dev['org.bluez.Device1'].Connect()
			print "*****connected******"

		else:
			print "already connected"

		self.en_notifications(sensor)

	def en_notifications(self, sensor):
		print "Enabling notifications..."
		SONAR_SVC_UUID
		char_addr = '/org/bluez/hci0/dev_DC_D3_01_DA_2D_3E/service000c/char000d'
		char = self.bus.get(BLUEZ_SVC_NAME, char_addr)
		char2 = char[GATT_SERVICE_IFACE]
		char.PropertiesChanged.connect(sensor.update_value_cb)
		#.InterfacesAdded.connect(self.new_iface)
